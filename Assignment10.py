...
rootFolder = "C:/Users/Clemens Jänicke/Python/Uni/data/Assignment10/"
...
# ####################################### START PROCESSING #################################################### #
# (1) get a list of tif files
rasList = general.GetFilesinFolderWithEnding(rootFolder, ".tif", fullPath=True)

# create a list of sublists
# in each sublist all elements are stored needed for each parallelization process
jobList = []
for ras in rasList:
...
# worker function that does everything needed in one parallel process
def workFunc(job):
    # all variables and functions that are needed for the process need to be inside this function
    # it can be seen as an independent script
    # nevertheless in some cases it is useful to define the variables inside and outside the worker function
    # because you might need them also outside the function
    drvR = gdal.GetDriverByName("GTiff")
